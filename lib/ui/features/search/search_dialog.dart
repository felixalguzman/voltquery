...
class _SearchDialogState extends ConsumerState<SearchDialog> {
  ...
  Future<void> _openHit(SchemaSearchHit hit) async {
    ...
    final target = dialect.qualify(hit.table.name, schema: hit.table.schema);
  ...
  Widget _field(bool loading) => Row(
    children: [
      Icon(FluentIcons.search, size: 14, color: t.textMid),
          ...
          child: HoverButton(
            ...
            builder: (context, states) => Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: _filter == f ? t.accent : t.hairline),
              ...
              child: Text(
                ...
                style: TextStyle(
                  fontSize: 10.5,
                  color: _filter == f ? t.accent : t.textMid,
                ),
              ),
            ),
          ),
  ...
  Widget _body(
    ...
    if (async.hasError) {
      return _Hint('Search failed: ${async.error}', color: t.danger);
    }
    ...
    return ListView(
      children: [
        if (results.schemaError != null)
          _Hint(
            'Could not search the schema: ${results.schemaError}',
            color: t.danger,
          ),
        if (tables.isNotEmpty) _group('TABLES & VIEWS', tables.length),
        ...
        Expanded(
          child: Divider(
            style: DividerThemeData(
              decoration: BoxDecoration(color: t.hairline),
            ),
          ),
        ),
      ],
  ...
  Widget _tile({
    ...
    return HoverButton(
      ...
      builder: (context, states) => Container(
        ...
        decoration: BoxDecoration(
          color: selected ? t.accentWash : (states.isHovered ? t.hover : null),
          borderRadius: BorderRadius.circular(4),
        ),
        child: Row(
          children: [
            Icon(icon, size: 12, color: selected ? t.accent : t.textMid),
            const SizedBox(width: 10),
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    ...
                    style: TextStyle(
                      color: selected ? t.accent : t.textHigh,
                      fontSize: 12.5,
                      fontFamily: 'monospace',
                    ),
                  ),
            ...
            if (onInfo != null) ...[
              ...
              Tooltip(
                message: 'Table info',
                child: IconButton(
                  icon: Icon(FluentIcons.info, size: 11, color: t.textLow),
                  onPressed: onInfo,
                ),
              ),
            ],
...
class _Hint extends StatelessWidget {
  ...
  /// Null means the default hint colour — resolved in [build], since a theme
  /// token is not a constant and so cannot be a parameter default.
  final Color? color;
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Text(
      text,
      style: TextStyle(
        color: color ?? VoltTheme.of(context).textLow,
        fontSize: 11.5,
        height: 1.4,
      ),
    ),
