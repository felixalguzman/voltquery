...
class SchemaRepository {
  ...
  Future<List<SchemaInfo>> schemas() => _memo(
      _schemas, 'schemas', () => introspector.schemas(const DatabaseInfo('')));
  ...
  Future<List<ColumnInfo>> columns(TableInfo table) => _memo(_columns,
      '${table.schema} ${table.name}', () => introspector.columns(table));

  Future<List<IndexInfo>> indexes(TableInfo table) => _memo(_indexes,
      '${table.schema} ${table.name}', () => introspector.indexes(table));

  /// The `CREATE` DDL for a table/view — backs "Copy CREATE" (#53). Cached like
  /// the rest; keyed apart from the index DDL by a `t:` prefix.
  Future<String> tableDdl(TableInfo table) => _memo(_ddl,
      't:${table.schema} ${table.name}', () => introspector.tableDdl(table));

  /// The `CREATE INDEX` DDL for an index — "Copy CREATE" on an index node (#53).
  Future<String> indexDdl(TableInfo table, IndexInfo index) => _memo(_ddl,
  ...
  /// for DDL).
  Future<T> _memo<T>(Map<String, Future<T>> cache, String key,
